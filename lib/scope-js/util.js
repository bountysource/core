with (scope()) {
  ...
  define('serialize_form', function(form) {
    ...
    flatten_to_array(form.getElementsByTagName('input')).map(function(input) {
      if (input.type == 'text') {
        var obj = { type: 'text' }; obj[input.name] = input.value;
        inputs.push(obj);
      } else if (input.type == 'checkbox') {
        var obj = { type: 'checkbox' }; obj[input.name] = (input.checked ? true : false);
        inputs.push(obj);
      }
    });
    flatten_to_array(form.getElementsByTagName('textarea')).map(function(textarea) {
      var obj = { type: 'textarea' }; obj[textarea.name] = textarea.value;
      inputs.push(obj);
    });
    flatten_to_array(form.getElementsByTagName('select')).map(function(select) {
      ...
      var obj = { type: 'select' }; obj[select.name] = { value: selected_option.value, text: selected_option.text };
      inputs.push(obj);
    });
    return inputs;
  });
