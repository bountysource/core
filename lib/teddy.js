...
with (scope('Teddy')) {
  define('snuggle', function(table_ref, options) {
    ...
    * */
    ...
    /*
    * Override #forEach and #map to use the Teddy#at method
    * */
    table_ref.map = function(method) { var arr=[]; for (var i=0; i<table_ref._t_rows.length; i++) arr.push(method(this.at(i))); return arr; };
    table_ref.forEach = function(method) { for (var i=0; i<table_ref._t_rows.length; i++) method(this.at(i)); };
