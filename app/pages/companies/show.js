  ...
  .config(function ($routeProvider) {
    $routeProvider
      .when('/companies/:id', {
        ...
        controller: 'EditCompaniesController'
      });
  })
  .controller('EditCompaniesController', function ($scope, $routeParams, $api) {
    $scope.project_search = null;

    $scope.company = $api.company_get($routeParams.id).then(function(company) {
      console.log(company);

      $scope.$init_company(company);

      return company;
    });

    $scope.$init_company = function(company) {
