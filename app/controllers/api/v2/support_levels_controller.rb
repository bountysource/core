  ...
  def index
    if params[:supporters_for_team]
      ...
      if params[:team_id]
        ...
        @collection = @collection.where(team: Team.where(slug:params[:team_slug]))             
      end
    end
  ...
  def paypal_return
    if params[:token]
      ...
      if temp_payment_method.data['support_level']['id']
        @item = current_user.support_levels.find(temp_payment_method.data['support_level']['id'])
      else
...
protected
  ...
  def create_or_update_support_level
    ...
    @item.assign_attributes(
      person: current_user,
      team: Team.accepts_public_payins.find(params[:team_id]),
    ...
    if params[:payment_method_id] == "paypal"
      ...
      if token = params[:payment_method_id].to_s.match(/^stripe:(.+)$/).try(:[],1)
        ...
        @item.payment_method = current_user.payment_methods.find(params[:payment_method_id])
      end
  ...
  def require_support_level
    @item = current_user.support_levels.find(params[:id])
  end
