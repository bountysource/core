...
class Pivotal::Issue < ::Issue
  ...
  def remote_sync(options={})
    ...
    api_options = {
      url: Pivotal::API.generate_base_url(self.tracker),
      path: Pivotal::API.generate_issue_detail_path(self)
    }
  ...
  def sync_comments_from_source
    api_options = {
      url: Pivotal::API.generate_base_url(self.tracker),
      path: Pivotal::API.generate_issue_comments_path(self)
    }
    api_response = Pivotal::API.fetch_issue_comments(api_options)
