...
class Jira::Issue < ::Issue
  ...
  def remote_sync(options={})
    ...
    api_options = {
      url: Jira::API.generate_base_url(tracker.url),
      path: Jira::API.generate_issue_path(tracker.url, number)
    }
    data = Jira::API.fetch_issue_list(api_options)
    tracker.sync_issues_from_array(data[:issues])
  end
