...
class Bugzilla::Issue < ::Issue
  belongs_to :tracker, class_name: "Bugzilla::Tracker", foreign_key: :tracker_id
  ...
  def remote_sync(options={})
    ...
    api_options = {
      url: Bugzilla::API.generate_base_url(tracker.url),
      path: Bugzilla::API.generate_issue_path(params)
    }
  ...
  def sanitized_body_html
    %(<div style="white-space: pre-wrap;">#{super}</div>)
  end
