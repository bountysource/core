...
class SourceForge::Tracker < ::Tracker

  has_many :issues, class_name: "SourceForge::Issue", foreign_key: :tracker_id
  ...
  def remote_sync(options={})
    ...
    results = SourceForge::API.fetch_issue_list(
      url: self.url,
      path: "?" + { limit: 100, page: 0 }.merge(options).to_param
    )
