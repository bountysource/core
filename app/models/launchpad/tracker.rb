...
class Launchpad::Tracker < ::Tracker

  has_many :issues, class_name: "Launchpad::Issue", foreign_key: :tracker_id
  MAX_RESULT_PER_PAGE = 75
  ...
  def remote_sync(options={})
    ...
    params = {
      "field.status:list" => ["NEW", "CONFIRMED", "INPROGRESS"], # only fetch open issues
      "limit" => MAX_RESULT_PER_PAGE, # fetch 100 issues for now
      "order" => "-important", # more important first
      "start" => start
    }
    api_options = {
      url: self.url,
      path: Launchpad::API.generate_issue_list_path(params)
    }
    api_response = Launchpad::API.fetch_issue_list(api_options)
