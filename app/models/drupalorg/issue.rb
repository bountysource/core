...
class Drupalorg::Issue < ::Issue
  attr_accessible :state, :priority, :number, :body
  belongs_to :tracker, class_name: "Drupalorg::Issue", foreign_key: :tracker_id
  ...
  def remote_sync(options={})
    ...
    api_options = {
      url: Drupalorg::API.generate_base_url(),
      path: Drupalorg::API.generate_issue_detail_path(self)
    }
    api_response = Drupalorg::API.fetch_issue(api_options)
  ...
  def sync_comments_from_source
    api_options = {
      url: Drupalorg::API.generate_base_url(self.tracker),
      path: Drupalorg::API.generate_issue_comments_path(self)
    }
    api_response = Drupalorg::API.fetch_issue_comments(api_options)

    # Remove nil body comments
    api_response.reject! { |comment| comment[:body_html].nil? }
