module HasCloudinary
  extend ActiveSupport::Concern

  class_methods do
    def has_cloudinary_image
      self.class_eval do
        ...
        before_validation do
          ...
          if input.blank?
            ...
            image = randomized_image(rand(108))
            self.cloudinary_id = "upload/#{image}"
          elsif (matches = input.match(/^#{ENV['CLOUDINARY_BASE_URL']}([a-z_]+)\/.*?([^\/]+)$/))
        ...
        def image_url(options={})
          ...
          if ( cloudinary_id =~ /github/ )
            ...
            "#{ENV['CLOUDINARY_BASE_URL']}#{container}/d_#{options[:default].split('/').last},c_pad,w_#{options[:size]},h_#{options[:size]},b_white/#{filename}"
          end
        end
