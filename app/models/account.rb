...
class Account < ActiveRecord::Base
  ...
  def self.transfers(date_range=nil)
    ...
    Split.joins(:boso_transaction, :account).where(created_at: date_range).where('transactions.type = ? AND splits.account_id = ?', 'Transaction::BankTransfer', instance.id).sum('splits.amount * -1')
  end
