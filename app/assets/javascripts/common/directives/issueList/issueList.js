angular.module('directives').directive('issueList', function() {
  return {
    restrict: "E",
    templateUrl: "common/directives/issueList/issueList.html",
    scope: {
      issues: "=",
    ...
    link: function(scope, element, attrs) {
      ...
      scope.hideAcceptingRequestForProposalIcon = Object.keys(attrs).indexOf('hideAcceptingRequestForProposalIcon') >= 0;
      ...
      scope.$changeSortOrder = function(column) {
        ...
        // always override certain orders
        if (['bounty', 'thumbs', 'participants'].indexOf(column) >= 0) {
          scope.$orderData.reverse = false;
        }
      ...
      if (angular.isArray(scope.include)) {
        ...
        scope.$showTeam = scope.include.indexOf('team') >= 0 || false;
