angular.module('app').controller('TeamBountiesController', function ($scope, $routeParams, $api, $pageTitle) {
  ...
  $scope.$watch('team', function(team) {
    if (team) {
      $pageTitle.set('Bounties', team.name, 'Bountysource');
    }
  });
  ...
  $api.team_bounties($routeParams.id).then(function(bounties) {
    ...
    for (var i=0; i<bounties.length; i++) {
      ...
      if (bounty.issue.paid_out) {
        ...
        var group_id = bounties[i].issue.tracker.team ? 'team-'+bounties[i].issue.tracker.team.slug : 'tracker-'+bounties[i].issue.tracker.slug;
        $scope.issue_group_hash[group_id] = $scope.issue_group_hash[group_id] || { bounty_total: 0.0, bounties: [] };
        $scope.issue_group_hash[group_id].bounty_total += bounties[i].amount;
        $scope.issue_group_hash[group_id].bounties.push(bounties[i]);
      }
    }
