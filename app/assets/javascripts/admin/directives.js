angular.module('app').
  ...
  directive('fundraiserCard', function() {
    return {
      ...
      templateUrl: "admin/fundraisers/partials/homepage_card.html"
    };
  }).
  directive('projectCard', function() {
    return {
      ...
      templateUrl: "admin/trackers/partials/homepage_card.html"
    };
  }).
        ...
        scope.$watch(attr.ownerHref, function(owner) {
          if (owner) {
            if ((/^person$/i).test(owner.type)) {
              element.attr("href", "/admin/people/"+owner.slug);
            } else if ((/^team(?:::[a-z]+)*$/i).test(owner.type)) {
              element.attr("href", "/admin/teams/"+owner.slug);
            }
          }
        });
      }
