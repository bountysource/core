...
describe Drupalorg::Issue do
  ...
  let(:tracker) { create(:drupalorg_project) }
  let(:issue) { create(:drupalorg_issue, tracker: tracker) }

  describe "extract_from_url" do
    let!(:issue) { create(:github_issue, url: "https://www.drupal.org/node/2755529", number: 2755529) }
    ...
    it "should find issue by URL" do
      ...
      Github::Issue.extract_from_url("https://www.drupal.org/node/#{issue.number}").should eq(issue)
      Github::Issue.extract_from_url("https://www.other-tracker.com/node/1337").should be_nil
    end
  end
