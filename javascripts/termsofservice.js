with (scope('TermsofService', 'App')) {
  ...
  route('#termsofservice', function() {
    render(
      ...
      p('THIS IS A LEGALLY BINDING AGREEMENT between you and BountySource ("', b('BountySource'), '", "', b('we'), '" or "', b('us'), '"). Checking the "I Agree" box during the registration process or by using the BountySource.com website ("', b('Site'), '") or any of the BountySource services ("', b('Services'), '") means that you agree to all the terms and conditions of this Terms of Service ("', b('Agreement'), '"). If you are entering into this Agreement on behalf of a company or other legal entity, you represent that you have the authority to bind such entity to these terms and conditions, in which case the terms "', b('you'), '" or "', b('your'), '" shall refer to such entity. If you do not have such authority, or if you do not agree with these terms and conditions, you must not proceed with the registration process or use our Site or Service.'),
      
      h1('1. Overview'),
      
      p('BountySource is a platform for encouraging open source projects by enabling users to offer cash rewards for solutions to open source issues posted on GitHub.com (“', b('GitHub'), '”).  BountySource is not affiliated with GitHub.'),
      
      ul(
          li('Any user can define an open source coding issue (“', b('Issue'), '”) by posting an issue/feature on GitHub.com.'),
          li('Any user can post a cash bounty (“', b('Bounty'), '”) on BountySource to encourage development of a successful solution to an Issue posted on GitHub. These users are referred to as "', b('Donors'), '" in this Agreement.'),
          ...
          li('Once Developers finish their solutions to an Issue (“', b('Solutions'), '”), they generate a merge request on GitHub. Persons authorized to accept pull requests on GitHub and merge codes will decide whether a Solution is accepted or not.  BountySource will pay the Bounty to the Developer of the first Solution accepted on GitHub.'),
          li('Users that are authorized to accept pull requests on GitHub are referred to as "', b('Committers'), '" in this Agreement.')
        ),
        
      h1('2. Bounties'),
      
      p('2.1 Donors must provide their payment at the time they create a Bounty. Donors consent to BountySource and its payments partners charging their payment card or other payment method for the amount of the Bounty when it is created.'),
      p('2.2 Donors may increase their Bounty (by creating another Bounty on the same Issue), but may not decrease or cancel their Bounty, after it has been created.'),
      p('2.3 When creating a Bounty, the Donor will set a deadline date/time by which a Solution must be accepted (the “', b('Deadline'), '”).'),
      p('2.4 Donors will receive 100% of their Bounty back if no Solution is accepted by the Deadline. Bounties are nonrefundable if a Solution is accepted by the Deadline.'),
      p('2.5 BountySource will pay Bounties to the Developer whose Solution is accepted by the Deadline. For all Bounties that are awarded, BountySource gives to the Developer each Donor’s screen name and pledge amount.'),
      p('2.6 Committers, Donors and Developers and other users should not take any action in reliance on having a successful Solution until the successful Solution has been accepted by a Committer on GitHub.'),
      ...
      p('2.8 Because of occasional failures of payments from Donors, BountySource cannot guarantee the full receipt of the Bounty amount pledged minus fees.'),
