...
describe("Scenario: Using Team Account --", function() {
  ...
  xdescribe("updating team settings", function() {
    ...
    it("should update slug", function() {
      input('form_data.slug').enter('test-team-infinity-two-point-o');
      using("form[ng-submit='create_team()']").element('submit[type=submit]').click();
      browser().navigateTo('/teams/'+MOCK.valid_team.slug+'/settings');
      expect(input('form_data.slug').val()).toEqual('test-team-infinity-two-point-o');
    });

    it("should update custom url", function() {
      input('form_data.url').enter('http://www.test-this-site-and-register-the-domain-before-someone-else-does.com');
      using("form[ng-submit='create_team()']").element('submit[type=submit]').click();
      browser().navigateTo('/teams/'+MOCK.valid_team.slug+'/settings');
      expect(input('form_data.url').val()).toEqual('http://www.test-this-site-and-register-the-domain-before-someone-else-does.com');
    });
  ...
  describe("adding/inviting members --", function() {
    ...
    it("add/invite member", function() {
      ...
      input('new_member.email').enter('test-teammate@bountysource.com');
